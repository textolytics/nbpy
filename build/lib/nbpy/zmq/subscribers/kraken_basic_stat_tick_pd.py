"""
KRAKEN_BASIC_STAT_TICK_PD Subscriber Module
Auto-migrated from python/scripts/zmq/sub_kraken_basic_stat_tick_pd.py
...
class KrakenBasicStatTickPdSubscriber(BasePandasSubscriber):
    """
    Subscriber for KRAKEN_BASIC_STAT_TICK_PD
    
    Original service: sub_kraken_basic_stat_tick_pd.py
    ...
    def __init__(self, connection_port=5558, topic_filter='1'):
        super().__init__(
            ...
            service_name='kraken_basic_stat_tick_pd'
        )
        self.codec = MessagePackCodec()
        logger.info(f"Initialized kraken_basic_stat_tick_pd Subscriber on port {self.connection_port}, topic: {self.topic_filter}")
    ...
    def process_message(self, topic, data):
        ...
        try:
            ...
            from python.scripts.zmq.sub_kraken_basic_stat_tick_pd import process_message
    ...
    def run(self):
        ...
        try:
            # Import and run original service logic
            from python.scripts.zmq.sub_kraken_basic_stat_tick_pd import run
            
            run()
            
        except ImportError:
            logger.error(f"Could not import original module: sub_kraken_basic_stat_tick_pd")
...
def main():
    """Entry point for kraken_basic_stat_tick_pd subscriber"""
    subscriber = KrakenBasicStatTickPdSubscriber()

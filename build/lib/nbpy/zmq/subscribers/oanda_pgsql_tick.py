"""
OANDA_PGSQL_TICK Subscriber Module
Auto-migrated from python/scripts/zmq/sub_oanda_pgsql_tick.py
...
class OandaPgsqlTickSubscriber(BasePostgreSQLSubscriber):
    """
    Subscriber for OANDA_PGSQL_TICK
    
    Original service: sub_oanda_pgsql_tick.py
    ...
    def __init__(self, connection_port=5558, topic_filter='1'):
        super().__init__(
            ...
            service_name='oanda_pgsql_tick'
        )
        self.codec = MessagePackCodec()
        logger.info(f"Initialized oanda_pgsql_tick Subscriber on port {self.connection_port}, topic: {self.topic_filter}")
    ...
    def process_message(self, topic, data):
        ...
        try:
            ...
            from python.scripts.zmq.sub_oanda_pgsql_tick import process_message
    ...
    def run(self):
        ...
        try:
            # Import and run original service logic
            from python.scripts.zmq.sub_oanda_pgsql_tick import run
            
            run()
            
        except ImportError:
            logger.error(f"Could not import original module: sub_oanda_pgsql_tick")
...
def main():
    """Entry point for oanda_pgsql_tick subscriber"""
    subscriber = OandaPgsqlTickSubscriber()

"""
KRAKEN_KAPACITOR_ORDERS Subscriber Module
Auto-migrated from python/scripts/zmq/sub_kraken_kapacitor_orders.py
...
from nbpy.zmq.base import BaseSubscriber, BaseKapacitorSubscriber
...
class KrakenKapacitorOrdersSubscriber(BaseKapacitorSubscriber):
    """
    Subscriber for KRAKEN_KAPACITOR_ORDERS
    
    Original service: sub_kraken_kapacitor_orders.py
    ...
    def __init__(self, connection_port=5558, topic_filter='1'):
        super().__init__(
            ...
            service_name='kraken_kapacitor_orders'
        )
        self.codec = MessagePackCodec()
        logger.info(f"Initialized kraken_kapacitor_orders Subscriber on port {self.connection_port}, topic: {self.topic_filter}")
    ...
    def process_message(self, topic, data):
        ...
        try:
            ...
            from python.scripts.zmq.sub_kraken_kapacitor_orders import process_message
    ...
    def run(self):
        ...
        try:
            # Import and run original service logic
            from python.scripts.zmq.sub_kraken_kapacitor_orders import run
            
            run()
            
        except ImportError:
            logger.error(f"Could not import original module: sub_kraken_kapacitor_orders")
...
def main():
    """Entry point for kraken_kapacitor_orders subscriber"""
    subscriber = KrakenKapacitorOrdersSubscriber()

"""
KRAKEN_EURUSD_DEPTH_SEND_ORDER Subscriber Module
Auto-migrated from python/scripts/zmq/sub_kraken_EURUSD_depth_send_order.py
...
class KrakenEurusdDepthSendOrderSubscriber(BaseSubscriber):
    """
    Subscriber for KRAKEN_EURUSD_DEPTH_SEND_ORDER
    
    Original service: sub_kraken_EURUSD_depth_send_order.py
    ...
    def __init__(self, connection_port=5558, topic_filter='1'):
        super().__init__(
            ...
            service_name='kraken_EURUSD_depth_send_order'
        )
        self.codec = MessagePackCodec()
        logger.info(f"Initialized kraken_EURUSD_depth_send_order Subscriber on port {self.connection_port}, topic: {self.topic_filter}")
    ...
    def process_message(self, topic, data):
        ...
        try:
            ...
            from python.scripts.zmq.sub_kraken_EURUSD_depth_send_order import process_message
    ...
    def run(self):
        ...
        try:
            # Import and run original service logic
            from python.scripts.zmq.sub_kraken_EURUSD_depth_send_order import run
            
            run()
            
        except ImportError:
            logger.error(f"Could not import original module: sub_kraken_EURUSD_depth_send_order")
...
def main():
    """Entry point for kraken_EURUSD_depth_send_order subscriber"""
    subscriber = KrakenEurusdDepthSendOrderSubscriber()

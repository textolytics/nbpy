...
# sudo bash new-service.sh "sub_twitter_influx_sentiment_location" "sub_twitter_influx_sentiment_location" "/usr/bin/python3.5 /home/zmq/nabla/python/scripts/zmq/sub_twitter_influx_sentiment_location.py" "root"
#-----------------------------------------------------------------------
...
negative_vocab = [line.strip() for line in open("/scripts/text/opinion_lexicon/negative-words.txt", 'r')]
positive_vocab = [line.strip() for line in open("/scripts/text/opinion_lexicon/positive-words.txt", 'r')]
...
def twitter(SeriesHelper):
    try:
        ...
        # turn the date string into a date object that python can handle
            ...
            myclient.write_points ( tweet_json , protocol='json' , time_precision='ms' )
            if coordinates != 'null':
